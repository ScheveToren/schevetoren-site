...
function friendlyApiError(error) {
  ...
  if (/urlfetch|fetchapp|external_request|backend_urlfetch/i.test(msg)) {
    return "De club-backend kan Lichess nog niet verifiëren (Google Apps Script: geen UrlFetch-rechten). Het bestuur moet het script eenmalig autoriseren — zie README → Google Apps Script.";
  }
  if (msg === "not_linked") return "Nog niet gekoppeld aan een speler.";
  if (msg === "forbidden") return "Geen toegang voor deze actie.";
  if (msg === "invalid or used invite code") return "Code ongeldig of al gebruikt.";
  return msg;
}
...
async function postJson(payload, retries = 2) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (data.message && (data.error === "backend_urlfetch" || data.error === "no_valid_rows" || data.error === "empty_attendance")) {
...
async function refreshSession() {
  ...
  updatePanels();
  setStatus("Accountstatus ophalen…");
  try {
...
async function loadPlayer() {
  ...
  try {
    ...
    setStatus(`Laden mislukt: ${friendlyApiError(error)}`, false);
  }
}
...
async function save() {
  ...
  try {
    ...
    if (!result.ok) throw Error(result.message || result.error || "Opslaan mislukt");
    if (!result.records) throw Error("Geen rijen opgeslagen — controleer speeldagen of koppeling.");
    const data = {};
    rows.forEach(row => {
      data[row.date] = row.status;
      data[`${row.date}-note`] = row.note;
...
async function bindEvents() {
  ...
  document.getElementById("linkPlayerBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      alert(`Koppelen mislukt: ${friendlyApiError(error)}`);
    }
  });
...
async function waitForLichessToken(maxMs = 10000) {
  const deadline = Date.now() + maxMs;
  while (Date.now() < deadline) {
    if (token()) return true;
    if (window.ScheveTorenAuth?.getAccount) {
      try {
        const user = await window.ScheveTorenAuth.getAccount();
