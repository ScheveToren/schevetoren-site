...
function getSheet(name) {
  ...
  if (!sheet) {
    throw new Error(
      `Missing sheet "${name}". Existing sheets: ` +
        spreadsheet
          .getSheets()
...
function getPlayers() {
  return sheetRowsAsObjects("Players");
}
...
function getAttendance(playerName) {
  ...
  return getAllAttendance().filter(row =>
    String(row.player_name || "").trim() === wantedPlayer
  );
}
...
function doGet(e) {
  ...
  if (action === "players") {
    return jsonResponse({ players: getPlayers() });
  }

  if (action === "attendance" && player) {
    return jsonResponse({ attendance: getAttendance(player) });
  }

  if (action === "export") {
    return jsonResponse({ rows: getAllAttendance() });
  }
...
function saveAttendanceRows(player, incomingRows) {
  ...
  if (sheet.getMaxRows() < output.length + 1) {
    sheet.insertRowsAfter(
      sheet.getMaxRows(),
      output.length + 1 - sheet.getMaxRows()
    );
  }

  sheet
    .getRange(1, 1, Math.max(sheet.getLastRow(), output.length + 1), 5)
    .clearContent();

  sheet.getRange(1, 1, 1, 5).setValues([
    SHEET_HEADERS.Attendance
  ]);
...
function linkPlayer(payload) {
  ...
  const existingIndex = playerRows.findIndex((row, index) => {
    return index > 0 && String(row[0] || "").trim() === player;
  });
...
function doPost(e) {
  ...
  try {
    ...
    if (action === "link-player") {
      return jsonResponse(linkPlayer(payload));
    }
    ...
    if (action === "save-attendance") {
      ...
      try {
        const count = saveAttendanceRows(player, incoming);
    ...
    if (action === "bulk-set") {
      ...
      if (!player || !["present", "absent"].includes(status)) {
        return jsonResponse({
          ok: false,
          error: "missing player or valid status"
        });
      }
      ...
      try {
        const count = saveAttendanceRows(player, rows);
