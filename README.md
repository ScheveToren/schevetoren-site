...
### CMS (website beheer voor admins)

Board members with `is_admin` can publish news, page JSON, and calendar ICS from [`docs/beheer.html`](docs/beheer.html). See **[docs/BESTUUR-CMS.md](docs/BESTUUR-CMS.md)** (Dutch) for day-to-day use.

**One-time IT setup:**

1. Paste latest [`scripts/google-apps-script-live.gs`](scripts/google-apps-script-live.gs) and redeploy the web app.
2. Apps Script → **Project settings → Script properties** → add `GITHUB_PAT` (GitHub token with **contents: write** on `ScheveToren/schevetoren-site` only). Never commit the PAT or put it in `docs/config.js`.
...
- `docs/contact.html`, `docs/jeugd.html`, `docs/gedragsregels.html` — from [`docs/content/pages/*.json`](docs/content/pages/) via [`scripts/generate-site-pages.py`](scripts/generate-site-pages.py)
